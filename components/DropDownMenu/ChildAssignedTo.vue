...
<template>
  <div class="teamMembersContainer position-relative">
    <div v-can:not="{perms: 'edit issue', location: appStore.permissionLocation}" class="cursor-default me-2">
    ...
    <div v-else @click="getMembers" v-can="{perms: 'edit issue', location: appStore.permissionLocation}">
      <TooltipMain v-if="assignedTo" :content='`Assigned to: "${assignedTo?.user?.name}"`' placement="right"
                   class="text-nowrap">
        <Avatar imgWidth="30" :user="assignedTo?.user"/>
      </TooltipMain>
    ...
    <Transition name="assignToMenu">
      <div v-if="assignOpen" v-click-outside="closeAssign" v-can="{perms: 'edit issue', location: appStore.permissionLocation}"
