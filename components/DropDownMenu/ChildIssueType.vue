...
const props = defineProps(["issueType", "projectId", "issueId", "from", "customStyle", "from"]);
...
const handleIssueType = async (projectId, issueId) => {
...
const handleChangeIssueType = async (issueTypeName) => {
  ...
  const response = await updateIssueInfoFromOutside(props.projectId, props.issueId, {type: issueTypeName});
  if (response?.code === 200) {
    if (props.from === "subIssue") {
      ...
      } else if (route.name === "projects-id") {
...
const issueImageSource = computed(() => {
  if (props.from === 'details' || isLoading.value) {
    return `/types/${props.issueType}.svg`;
  } else {
    return `/types/${props.issueType}.svg`;
  }
});
...
<template>
  <div class="position-relative issueType-menu-parent" v-click-outside="closeMenu" @click="toggleIssueTypeMenu"
    ...
    <div v-else class="d-flex align-items-center fs-4 cursor-pointer custom-round type-show"
         @click="handleIssueType(projectId, issueId);" :class="[{ active: isActive }, customStyle ? customStyle : '']">
      <img :src="issueImageSource" alt="Issue" width="14" v-if="from === 'subIssue' && !isLoading">
      <img :src="issueImageSource" alt="Issue" width="14" v-if="from !== 'subIssue'">
    ...
    <ul class="position-absolute flex-column bg-white shadow-sm gap-2 mb-0 custom-width options z-5 issueType-menu list-style-none rounded-1"
        :class="{ active: isActive && !isLoading}" v-if="isActive && !isLoading" style="z-index: 10;">
